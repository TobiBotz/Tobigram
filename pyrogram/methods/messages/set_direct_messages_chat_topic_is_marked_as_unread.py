...
class SetDirectMessagesChatTopicIsMarkedAsUnread:
    async def set_direct_messages_chat_topic_is_marked_as_unread(
        ...
        Parameters:
            ...
            topic_id (``int``):
                Identifier of the topic to mark as read or unread.
        ...
        Example:
            .. code-block:: python
                ...
                await app.set_direct_messages_chat_topic_is_marked_as_unread(chat_id, topic_id, False)
        """
        return await self.invoke(
            raw.functions.messages.MarkDialogUnread(
                parent_peer=await self.resolve_peer(chat_id),
                peer=raw.types.InputDialogPeer(peer=await self.resolve_peer(topic_id)),
