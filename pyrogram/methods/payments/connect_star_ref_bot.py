...
class ConnectStarRefBot:
    async def connect_star_ref_bot(
        ...
        """Connect a Star affiliate referral bot to a channel.
        ...
        Parameters:
            ...
            bot_id (``int`` | ``str``):
                Bot identifier or username to connect.
        ...
        Example:
            .. code-block:: python
                ...
                res = await app.connect_star_ref_bot(channel_id, "@affiliate_bot")
        """
        peer = await self.resolve_peer(chat_id)
        bot_peer = await self.resolve_peer(bot_id)

        return await self.invoke(
            raw.functions.payments.ConnectStarRefBot(
                peer=peer,
                bot=bot_peer,
            )
        )
