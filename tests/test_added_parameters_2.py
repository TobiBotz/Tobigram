...
@pytest.mark.asyncio
async def test_recover_gaps_skips_peer_with_no_pts():
    class Storage:
        async def update_state(self, value=None):
            return [(0, None, 1, 1, 1), (-100, None, 1, 1, 1)] if value is None else None
    ...
    assert await pyrogram.Client.recover_gaps(client) == (0, 0)
    assert client.sent == []
...
async def test_passing_none_sends_an_inline_markup_with_no_rows():
    ...
    await pyrogram.Client.edit_message_reply_markup(
        client, chat_id=7, message_id=11, reply_markup=None
    )
