...
def test_unknown_thumbnail_source():
    unknown = (
        "AAMCAgADHQIAAaCv0AACCY9ftvhTFuM2gUQ78vQjizrCtc0kDwACBAADgaS5Sb0UW8ho6FdZIH3qihAAA6QBAAIeBA"
    )
...
def test_secure_encrypted_temp_unique_id():
    for ftype in (FileUniqueType.SECURE, FileUniqueType.ENCRYPTED, FileUniqueType.TEMP):
        original = FileUniqueId(file_unique_type=ftype, media_id=5312458109417947140)
        encoded = original.encode()
        decoded = FileUniqueId.decode(encoded)

        assert decoded.file_unique_type == ftype
        assert decoded.media_id == 5312458109417947140
