...
def test_html_unparse_mixed():
    expected = (
        "<b>aaaaaaa<i>aaa<u>bbbb</u></i></b><u><i>bbbbbbccc</i></u><u>ccccccc<s>ddd</s></u><s>ddddd<tg-spoiler>dd"
...
def test_html_unparse_blockquote_expandable():
    ...
    assert (
        HTML.unparse(text=text, entities=entities)
...
def test_html_unparse_pre_no_language():
    ...
    entities = pyrogram.types.List(
        [
            pyrogram.types.MessageEntity(
                type=pyrogram.enums.MessageEntityType.PRE, offset=0, length=10
            )
        ]
    )
...
def test_html_parse_emoji_id():
    html = HTML(client=None)
    import asyncio

    async def run():
        # Test <emoji id="...">
        result = await html.parse("<emoji id='4956222745814762495'>❤️</emoji>")
        assert len(result["entities"]) == 1
        assert result["entities"][0].document_id == 4956222745814762495

        # Test nested in bold
        result = await html.parse("<b><emoji id='4956222745814762495'>❤️</emoji></b>")
