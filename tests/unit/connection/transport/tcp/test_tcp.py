...
def test_tcp_is_mtproxy_for_mtproxy():
    mtproxy = MTProxy(hostname="1.2.3.4", port=443, secret=bytes.fromhex(PLAIN_SECRET_HEX))
    transport = TCP(proxy=mtproxy, dc_id=2)
    assert transport.is_mtproxy
    assert not transport.is_web_proxy


async def test_connect_via_mtproxy_requires_dc_id():
    mtproxy = MTProxy(hostname="1.2.3.4", port=443, secret=bytes.fromhex(PLAIN_SECRET_HEX))
    transport = TCPAbridged(proxy=mtproxy, dc_id=None)
    with pytest.raises(ValueError, match="dc_id"):
        await transport._connect_via_mtproxy()


async def test_connect_via_mtproxy_requires_an_obfuscate_tag():
    mtproxy = MTProxy(hostname="1.2.3.4", port=443, secret=bytes.fromhex(PLAIN_SECRET_HEX))
    transport = TCP(proxy=mtproxy, dc_id=2)
    with pytest.raises(ValueError, match="OBFUSCATE_TAG"):
        await transport._connect_via_mtproxy()
...
async def test_build_proxy_configures_python_socks_for_socks5():
    transport = TCP(
        proxy=SOCKS5Proxy(hostname="127.0.0.1", port=1080, username="user", password="pwd")
    )
    proxy = await transport._build_proxy()

    assert proxy._proxy_type == ProxyType.SOCKS5
    assert proxy._proxy_host == "127.0.0.1"
    assert proxy._proxy_port == 1080
    assert proxy._username == "user"
    assert proxy._password == "pwd"


async def test_build_proxy_configures_python_socks_for_socks4():
    transport = TCP(proxy=SOCKS4Proxy(hostname="127.0.0.1", port=1080))
    proxy = await transport._build_proxy()

    assert proxy._proxy_type == ProxyType.SOCKS4
    assert proxy._proxy_host == "127.0.0.1"
    assert proxy._proxy_port == 1080


async def test_build_proxy_configures_python_socks_for_http():
    transport = TCP(proxy=HTTPProxy(hostname="127.0.0.1", port=8080))
    proxy = await transport._build_proxy()

    assert proxy._proxy_type == ProxyType.HTTP
    assert proxy._proxy_host == "127.0.0.1"
    assert proxy._proxy_port == 8080


async def test_build_proxy_rejects_non_dialed_proxy():
    mtproxy = MTProxy(hostname="1.2.3.4", port=443, secret=bytes.fromhex(PLAIN_SECRET_HEX))
    transport = TCP(proxy=mtproxy)
    with pytest.raises(ValueError, match="cannot be dialed as a SOCKS/HTTP proxy"):
